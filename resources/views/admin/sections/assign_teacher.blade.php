                    ...
                    @if ($section->classTeacher)
                        <div class="flex items-center justify-between">
                            ...
                            <form method="POST" action="{{ route('finance.sections.assign_teacher.remove', $section->id) }}"
                                  onsubmit="return confirm('Are you sure you want to remove this teacher from this class?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="flex items-center text-sm font-medium text-red-600 bg-red-50 hover:bg-red-100 px-4 py-2 rounded-lg transition-colors">
